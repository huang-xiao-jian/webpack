...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
	...
	_addModule(module, callback) {
		// 关键属性 _modules Map 类型，存放 identity --> 模块实例
		// 关键属性 modules Set 类型，存放模块实例
		// 可能关键属性 _modulesCache Map 类型 也存放模块实例
		...
		this.factorizeModule(
			...
			(err, newModule) => {
				...
				this.addModule(newModule, (err, module) => {
					...
					// original module --> dependency --> module，暂且理解为单向
	...
	/**
	 * @description - 基于 dependency 扩展 module graph，dependency 类型不重要
	 *
